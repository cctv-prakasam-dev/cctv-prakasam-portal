...
export const VCreateVideoSchema = pipeAsync(
  object({
    youtube_id: pipe(
      string("YouTube ID must be a string"),
      nonEmpty("YouTube ID is required"),
      maxLength(20, "YouTube ID must be at most 20 characters"),
    ),
    title: pipe(
    ...
    title_te: optional(pipe(
      string("Telugu title must be a string"),
      maxLength(500, "Telugu title must be at most 500 characters"),
    )),
    description: optional(string("Description must be a string")),
    category_id: optional(pipe(
      number("Category ID must be a number"),
      integer("Category ID must be an integer"),
    )),
    thumbnail_url: optional(pipe(
      string("Thumbnail URL must be a string"),
      maxLength(500, "Thumbnail URL must be at most 500 characters"),
    )),
    duration: optional(pipe(
      string("Duration must be a string"),
      maxLength(10, "Duration must be at most 10 characters"),
    )),
    view_count: optional(pipe(
      string("View count must be a string"),
      maxLength(20, "View count must be at most 20 characters"),
    )),
