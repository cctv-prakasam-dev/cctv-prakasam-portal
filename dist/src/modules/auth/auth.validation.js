...
export const VRegisterSchema = pipeAsync(object({
    ...
    if (emailAlreadyExists) {
        prepareValibotIssue(dataset, addIssue, "email", emailValue, "An account with this email already exists");
    }
    return dataset.value;
}));
export const VLoginSchema = object({
    email: pipe(string("Email must be a string"), nonEmpty("Email is required"), email("Invalid email format")),
    password: pipe(string("Password must be a string"), nonEmpty("Password is required")),
});
export const VRefreshTokenSchema = object({
    refresh_token: pipe(string("Refresh token must be a string"), nonEmpty("Refresh token is required")),
});
